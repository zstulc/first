...
class index extends foreground {
	...
	public function  checkbaoming(){
		...
		//get_one($where = '', $data = '*', $order = '', $group = '')返回单条数据
			
			$userinfo = array();
			$name=$userinfo['username']=$this->memberinfo['username'];
			$userinfo['realname'] = (isset($_POST['realname']) && is_username($_POST['realname'])) ? $_POST['realname'] :showmessage("姓名未填写", 'index.php?m=member&c=index&a=baoming');
			$userinfo['phone'] = (isset($_POST['phone']) && is_username($_POST['phone'])) ? $_POST['phone'] : showmessage("手机未填写", 'index.php?m=member&c=index&a=baoming');
			$userinfo['qq'] = (isset($_POST['qq']) && is_username($_POST['qq'])) ? $_POST['qq'] : showmessage("QQ未填写", 'index.php?m=member&c=index&a=baoming');
			$test= pc_base::load_model('member_test_model');
			$date=$test->get_one(array('username'=>$name));
			
			if($date){
				showmessage("您已报名，请勿重复报名", 'index.php?m=member&c=index&a=init');
			}else{
